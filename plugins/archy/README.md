...
The plugin provides 21 MCP tools for graph/rule queries, similarity and reuse guidance, planning, readiness, decisions, and sessions. Their descriptions and JSON input schemas are published through MCP `tools/list`. The hooks and advisory tools do not claim to prevent a file write. Use `archy verify` and required CI for delivery-boundary enforcement.
