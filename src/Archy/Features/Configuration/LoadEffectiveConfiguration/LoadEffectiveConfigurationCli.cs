...
public static partial class LoadEffectiveConfigurationCli
{
    ...
    private static void WriteSuccess(bool json, EffectiveConfiguration configuration)
    {
        if (json)
        {
            Console.WriteLine(JsonSerializer.Serialize(
                configuration,
                LoadEffectiveConfigurationJsonContext.Default.EffectiveConfiguration));
            return;
        }
    ...
    private static void WriteError(bool json, Problem problem)
    {
        ...
        {
            Console.WriteLine(JsonSerializer.Serialize(
                new LoadEffectiveConfigurationErrorResponse(problem.Code, problem.Message),
                LoadEffectiveConfigurationJsonContext.Default.LoadEffectiveConfigurationErrorResponse));
            return;
        }
    ...
    private sealed record LoadEffectiveConfigurationErrorResponse(string Code, string Message);

    [JsonSerializable(typeof(EffectiveConfiguration))]
    [JsonSerializable(typeof(LoadEffectiveConfigurationErrorResponse))]
    private sealed partial class LoadEffectiveConfigurationJsonContext : JsonSerializerContext;
}
