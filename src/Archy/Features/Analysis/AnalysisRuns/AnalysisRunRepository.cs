...
public sealed partial class AnalysisRunRepository(TimeProvider timeProvider, IWorkspaceLockManager lockManager) : IAnalysisRunRepository
{
    public ValueTask<Result<AnalysisRun>> StartAsync(
        WorkspaceStateLocation location,
        string analyzerVersion,
        string configurationHash,
        string? repositoryCommit,
        CancellationToken cancellationToken) =>
        StartCoreAsync(location, analyzerVersion, configurationHash, repositoryCommit, repositoryProvenance: null, cancellationToken);
    ...
    public ValueTask<Result<AnalysisRun>> StartAsync(
        ...
        CancellationToken cancellationToken)
    {
        ArgumentNullException.ThrowIfNull(repositoryProvenance);
        return StartCoreAsync(
            location,
            analyzerVersion,
            configurationHash,
            repositoryProvenance.HeadCommit,
            repositoryProvenance,
            cancellationToken);
    }

    private async ValueTask<Result<AnalysisRun>> StartCoreAsync(
        WorkspaceStateLocation location,
        string analyzerVersion,
        string configurationHash,
        string? repositoryCommit,
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (repositoryProvenance is not null)
            {
                await ExecuteAsync(
                    connection,
                    transaction,
                    "INSERT INTO analysis_run_repository_provenance(run_id, head_commit, worktree_state, changed_paths_json, captured_at_utc) VALUES ($runId, $headCommit, $worktreeState, $changedPathsJson, $capturedAt);",
                    cancellationToken,
                    ("$runId", run.RunId),
                    ("$headCommit", repositoryProvenance.HeadCommit),
                    ("$worktreeState", ToDatabase(repositoryProvenance.WorktreeState)),
                    ("$changedPathsJson", JsonSerializer.Serialize(repositoryProvenance.ChangedPaths.ToArray(), AnalysisRunJsonContext.Default.StringArray)),
                    ("$capturedAt", run.StartedAtUtc.ToString("O")));
            }
    ...
    private static string ToDatabase(RepositoryWorktreeState state) => state.ToString().ToLowerInvariant();

    [System.Text.Json.Serialization.JsonSerializable(typeof(string[]))]
    private sealed partial class AnalysisRunJsonContext : JsonSerializerContext;
}
