...
/// <summary>Builds bounded method-only chunks from the active graph; paths are resolved under the repository root before any file read.</summary>
public sealed class EmbeddingIndexSourceReader(IGraphRevisionSnapshotReader snapshots, IImportantNodeEligibilityPolicy eligibilityPolicy, IEmbeddingChunkSelector selector) : IEmbeddingIndexSourceReader
{
    ...
    public async ValueTask<Result<EmbeddingIndexPlan>> ReadAsync(WorkspaceStateLocation location, string repositoryRoot, ArchyConfiguration configuration, int maxChunks, CancellationToken cancellationToken)
    {
        ...
        foreach (var node in snapshot.Value.Nodes.Where(IsMethod).OrderBy(static node => node.StableId, StringComparer.Ordinal))
        {
    ...
    private static bool IsMethod(Archy.Features.Graph.CommitGraphRevision.GraphNodeFact node) => string.Equals(node.NodeKind, "method", StringComparison.OrdinalIgnoreCase) || string.Equals(node.NodeKind, "semantic_method", StringComparison.OrdinalIgnoreCase);
}
